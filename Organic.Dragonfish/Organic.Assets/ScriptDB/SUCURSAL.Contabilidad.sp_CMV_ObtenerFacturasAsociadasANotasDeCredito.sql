IF EXISTS(SELECT * FROM sys.objects WHERE name = 'sp_CMV_ObtenerFacturasAsociadasANotasDeCredito' AND type in (N'P') and SCHEMA_ID('Contabilidad') = schema_id)
	DROP PROCEDURE [Contabilidad].[sp_CMV_ObtenerFacturasAsociadasANotasDeCredito];
GO;

CREATE PROCEDURE [Contabilidad].[sp_CMV_ObtenerFacturasAsociadasANotasDeCredito]
	( 
...
AS
BEGIN
	/* Aqui ciclo entre las bases de dados de un agrupamiento y voy obteniendo las facturas originales que generaron las notas de credito */
	...
	--declare @ParamTablaDeRetorno varchar(100) = '##tblTmpCompAsociados'
	...
	/*----------------------------------------------------------------------*/
	--DECLARE	@tblTmpCompAsociados Contabilidad.udt_TableType_CMVCompAsociados
	--SELECT * INTO ##tblTmpCompAsociados FROM @tblTmpCompAsociados
	/*----------------------------------------------------------------------*/
	...
	BEGIN
		...
		SET @Sql = @Sql + 'EXEC [Contabilidad].[sp_CMV_ObtenerCompAsociadosANotasDeCredito] ''' + @CatalogoSQL + ''', ''' + @FechaDesde + ''', ''' + @FechaHasta + ''' '
	...
	/*-------------------------------------*/
	--SELECT * FROM ##tblTmpCompAsociados
	--DROP TABLE ##tblTmpCompAsociados
	/*-------------------------------------*/


	--EXEC [Contabilidad].[sp_CMV_ObtenerFacturasAsociadasANotasDeCredito] 'DRAGONFISH', 'BASESCMV', '20250401', '20250430', '##NombreTabla'
