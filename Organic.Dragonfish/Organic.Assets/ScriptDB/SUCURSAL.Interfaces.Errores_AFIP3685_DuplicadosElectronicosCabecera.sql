IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Interfaces].[Errores_AFIP3685_DuplicadosElectronicosCabecera]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
	DROP FUNCTION [Interfaces].[Errores_AFIP3685_DuplicadosElectronicosCabecera];
GO;

CREATE FUNCTION [Interfaces].[Errores_AFIP3685_DuplicadosElectronicosCabecera]
( 
...
(
	select distinct case when DatosExpo.ConversionTipoComprobante is null then cast( DatosExpo.TipoComprobante as varchar(2) ) + DatosExpo.LetraComprobante else null end as TipoComprobante,
		...
		case when DatosExpo.ConversionSituacionFiscal is null then DatosExpo.TipoResponsable else null end as TipoResponsable
	from ( 
		select
			Comprobante.FactTipo as TipoComprobante,
			cast( Comprobante.Fletra as char(1) ) as LetraComprobante,
			case when Comprobante.Anulado = 1 then 'PESOS' else Comprobante.Moneda end as Moneda,
			Cliente.ClTipoDoc as TipoDocumento,
			ConvVTipo.ValDest as ConversionTipoComprobante,
			ConvVMone.ValDest as ConversionMoneda,
			ConvVSitF.ValDest as ConversionSituacionFiscal,
			...
			case when Comprobante.Anulado = 1 then '3' else cast( Comprobante.SitFiscCli as varchar(2) ) end as TipoResponsable,
	...
	where DatosExpo.ConversionTipoComprobante is null
		...
		or DatosExpo.ConversionSituacionFiscal is null 
