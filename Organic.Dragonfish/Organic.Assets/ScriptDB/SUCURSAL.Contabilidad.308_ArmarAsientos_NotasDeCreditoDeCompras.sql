...
GO;
Create Function [Contabilidad].[ArmarAsientos_NotasDeCreditoDeCompras]
	(
...
(
	select 
		...
		convert(char(150), 'Compras - NC ' + cab.fletra + ' ' + right( replicate( '0', 4 ) + ltrim(str( cab.fptoven )), 4 ) + '-' + right( replicate( '0', 8 ) + ltrim(str( cab.fnumcomp )), 8 ) + '  [' + cab.fperson + ' - ' + isnull(prov.clnom,'') + ']' ) as Referencia,
		cab.codigo,
		...
		case when asietipo.CodConc = 'COMPRAS' and detCompras.Monto is not null then detCompras.Monto * cab.cotiz
			 ...
			 when asietipo.CodConc = 'IVACREDITO' then 0
			 ...
			 when asietipo.CodConc = 'IVADEBITO' then cab.fimpuesto * cab.cotiz * ( -1 )
	 ...
	 from ZooLogic.NCCOMPRA cab
	  ...
	  left join ( select det.codigo, impdir.cuentaDebe, impdir.cuentaHaber,
						...
						from ( select d.codigo, d.fart, d.PrunSinImp, d.fcant,
								...
								from ZooLogic.NCCOMPRADET d
								 inner join ZooLogic.NCCOMPRA c on d.codigo = c.codigo
	  ...
	  left join ( select val.jjnum, 
				  ...
				  from ZooLogic.VALNCCOMP val
					inner join ZooLogic.NCCOMPRA c on val.JJNUM = c.codigo
