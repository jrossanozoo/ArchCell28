IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Funciones].[ObtenerIVADelComprobanteAsociadoAlMovimientoDeCaja]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [Funciones].[ObtenerIVADelComprobanteAsociadoAlMovimientoDeCaja];
GO;

CREATE FUNCTION [Funciones].[ObtenerIVADelComprobanteAsociadoAlMovimientoDeCaja]
	( 
	...
	if @GUID is null
		...
		set @retorno = case @Tabla 
							when 'COMPROBANTEV' then 
								(
								select cast( cv.SIGNOMOV * cv.FIMPUESTO as numeric( 15, 2 ) )
							...
							when 'FACCOMPRA' then 
								(
								select cast( fc.SIGNOMOV * fc.FIMPUESTO as numeric( 15, 2 ) )
							...
							when 'NDCOMPRA' then 
								(
								select cast( ndc.SIGNOMOV * ndc.FIMPUESTO as numeric( 15, 2 ) )
							...
							when 'NCCOMPRA' then 
								(
								select cast( ncc.SIGNOMOV * ncc.FIMPUESTO as numeric( 15, 2 ) )
