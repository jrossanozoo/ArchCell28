...
GO;
Create Function [Contabilidad].[ArmarAsientos_NotasDeDebitoDeCompras]
	(
...
(
	select 
		...
		convert(char(150), 'Compras - ND ' + cab.fletra + ' ' + right( replicate( '0', 4 ) + ltrim(str( cab.fptoven )), 4 ) + '-' + right( replicate( '0', 8 ) + ltrim(str( cab.fnumcomp )), 8 ) + '  [' + cab.fperson + ' - ' + isnull(prov.clnom,'') + ']' ) as Referencia,
		cab.codigo,
	 ...
	 from ZooLogic.NDCOMPRA cab
	  ...
	  left join ( select det.codigo, impdir.cuentaDebe, impdir.cuentaHaber,
						...
						from ( select d.codigo, d.fart, d.PrunSinImp, d.fcant,
								...
								from ZooLogic.NDCOMPRADET d
								 inner join ZooLogic.NDCOMPRA c on d.codigo = c.codigo
	  ...
	  left join ( select val.jjnum, 
				  ...
				  from ZooLogic.VALNDCOMP val
					inner join ZooLogic.NDCOMPRA c on val.JJNUM = c.codigo
