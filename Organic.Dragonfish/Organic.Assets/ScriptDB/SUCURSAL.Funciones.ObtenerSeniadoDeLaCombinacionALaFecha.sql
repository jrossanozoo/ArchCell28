IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Funciones].[ObtenerSeniadoDeLaCombinacionALaFecha]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [Funciones].[ObtenerSeniadoDeLaCombinacionALaFecha];
GO;

CREATE FUNCTION [Funciones].[ObtenerSeniadoDeLaCombinacionALaFecha]
	(
	@Articulo char(15),
	@Color char(6),
	@Talle char(5),
...
BEGIN
	...
	set @retorno =	(select top 1           
	    cast( c_COMB.SENIADO - case when c_ADT_COMB.seniado  is null then 0 else c_ADT_COMB.seniado end as numeric(15, 2) ) as Cantidad
					from ZooLogic.COMB as c_COMB
						left join 
							( 
								select 
									COCOD, 
									sum(SENIADO) as SENIADO
