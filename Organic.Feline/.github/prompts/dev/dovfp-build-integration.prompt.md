---
description: "Integración con DOVFP - compilador Visual FoxPro para VS Code"
mode: "agent"
tools: ["read_file", "run_in_terminal", "grep_search", "file_search"]
---

# 🛠️ Integración con DOVFP Build System
...
- Ejecuta archivos PRG con parÃ¡metros
...
### 1. Compilar proyecto o soluciÃ³n
...
# Compilar con versiÃ³n especÃ­fica
...
# Compilar con encriptaciÃ³n
dovfp build -build_encrypted 1
```

**Opciones disponibles**:
- `-path`: Ruta al proyecto (.vfpproj), soluciÃ³n (.vfpsln) o directorio
- `-output_path`: Ruta de salida (por defecto bin/)
- `-build_force`: 1=forzar recompilaciÃ³n, 0=incremental (default)
- `-build_debug`: 1=Debug (default), 2=Release
- `-build_encrypted`: 1=compilar con encriptaciÃ³n, 0=normal (default)
...
# Ejecutar desde ruta especÃ­fica
...
# Restaurar de proyecto/soluciÃ³n especÃ­fica
dovfp restore -path Organic.Drawing.vfpsln

# Forzar re-descarga de paquetes
dovfp restore -force_download 1

# Con token de autenticaciÃ³n para feeds privados
dovfp restore -feed_token "tu_token_aqui"
```

**Opciones disponibles**:
- `-path`: Ruta al proyecto (.vfpproj), soluciÃ³n (.vfpsln) o directorio
- `-force_download`: 1=forzar descarga, 0=usar cachÃ© (default)
...
**Nota**: La funcionalidad de testing estÃ¡ en desarrollo. Consultar documentaciÃ³n actualizada.

### 5. Limpiar builds

```bash
# Limpiar archivos de compilaciÃ³n (bin/, obj/, packages/) y cachÃ© NuGet
dovfp clean
...
# Rebuild de proyecto especÃ­fico
...
**Nota sobre archivos de proyecto**: Los archivos `.vfpsln` y `.vfpproj` tienen formatos especÃ­ficos del proyecto. Consulta ejemplos existentes en el workspace para entender su estructura.
...
### Problema: Error de autenticaciÃ³n con Azure DevOps
...
**SoluciÃ³n**:
```powershell
# OpciÃ³n 1: Azure CLI (recomendado)
az login
az account show

# OpciÃ³n 2: PAT (Personal Access Token) via Azure Key Vault
# NO hardcodear tokens en archivos

# OpciÃ³n 3: Usar API REST directa (implementado en extensiÃ³n)
# Ver: src/services/dovfpService.js
```

### Problema: CompilaciÃ³n falla pero no muestra errores
...
**Causas comunes**:
1. Rutas absolutas en cÃ³digo
2. Dependencias de archivos locales
3. Diferencias de configuraciÃ³n regional
...
* ConfiguraciÃ³n regional explÃ­cita
SET DATE TO BRITISH
SET CENTURY ON
SET DECIMALS TO 2
...
- [ ] `PackageReference` con versiones especÃ­ficas (no wildcards)
...
@workspace #file:azure-pipelines.yml Optimiza este pipeline para builds mÃ¡s rÃ¡pidos con DOVFP

@workspace DOVFP estÃ¡ fallando con error de dependencias, ayÃºdame a diagnosticar
...
- DocumentaciÃ³n DOVFP: `/docs/quick-start/dovfp-guide.md`
