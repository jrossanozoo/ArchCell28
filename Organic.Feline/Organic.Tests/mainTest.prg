...
TRY
	...
	*-- Agregar clasesdeprueba al PATH para que los tests encuentren archivos de prueba (ej: ControlABMReplica.xml)
	SET PATH TO "clasesdeprueba" additive
	
	goDovfpTestInit.RegistrarMockSetupRegistry("OrganicFelineMockSetupRegistry", "OrganicFelineMockSetupRegistry.prg")

	InstanciarMocksGlobales()

	goDovfpTestInit.InstanciarZooMock()
	goDovfpTestInit.InstanciarZooAplicacion("Felino")
	
	_screen.Zoo.App.cSucursalActiva = "FELINO"

	*-- Pasos de AplicacionFelino.Iniciar() que InstanciarZooAplicacion() no ejecuta.
	*-- InstanciarZooAplicacion() llama los pasos de AplicacionBase paso a paso,
	*-- pero no llama .Iniciar(), asi que los pasos Felino-especificos no se ejecutan.
	*-- Los ejecutamos aqui, envolviendo en TRY los que pueden fallar por dependencias.
	WITH _Screen.Zoo.App
		&& *-- 1. Variables PUBLIC que AplicacionFelino.Iniciar() declara
		&& PUBLIC glProcesaAPSA, glProcesaTOTALSALE, glProcesaPUNTASHOPPING, ;
		&& 	glProcesaCABALLITOSHOPPING, glProcesarVENTASFISERV, glModuloVentasFiServ
		&& glProcesaAPSA = .F.
		...
		*-- AgregarConfiguradoresAdicionalesAlAgente, EnlazarParametros e IniciarBascula
		*-- son metodos PROTECTED de AplicacionFelino, no se pueden llamar desde afuera.
	ENDWITH

	*-- goControladorFiscal es declarado PUBLIC por IniciarControlador() pero queda NULL en test env
	*-- (sin hardware fiscal). Tests que acceden a .oCaracteristicas sin null-check crashean.
	*-- Fix: crear mock LOOSE de ComponenteFiscalManualArgentina (clase con stub en _dovfp_stubs)
	*-- y configurar oCaracteristicas via SetupGet. En TRY propio: si falla, queda NULL (seguro).
	IF VARTYPE(goControladorFiscal) != "O" OR ISNULL(goControladorFiscal)
		LOCAL loMockCF, loMockCaract, loSetupCF
		loMockCF    = goDoVfpMock.Mock("ComponenteFiscalManualArgentina", "ComponenteFiscalManualArgentina.prg", "LOOSE")
		loMockCaract = goDoVfpMock.Mock("ComponenteFiscalManualArgentina", "ComponenteFiscalManualArgentina.prg", "LOOSE")
		loSetupCF   = loMockCF.SetupGet("oCaracteristicas")
		loSetupCF.Returns(loMockCaract.Object)
		goControladorFiscal = loMockCF.Object
	ENDIF
	...
	*-- Asegurar Pais != 2 (Chile) para evitar validacion de Factura Electronica en tests.
	*-- ent_valor.prg activa lHabilitarFacturaelectronica cuando Pais == 2.
	TRY
		IF VARTYPE(goParametros) = "O" AND ;
		   PEMSTATUS(goParametros.Nucleo.DatosGenerales, "Pais", 5)
			IF goParametros.Nucleo.DatosGenerales.Pais = 2
				goParametros.Nucleo.DatosGenerales.Pais = 1
			ENDIF
		ENDIF
	CATCH
		*-- Ignorar: si la estructura de goParametros no permite el acceso directo
	ENDTRY
...
FUNCTION InstanciarMocksGlobales() As Void
	...
	*-- ManagerEjecucion (DEBE ir al principio para evitar inicializaci�n de AAO)
	*-- Al devolver TieneScriptCargado() = .T., se omite la verificaci�n del AAO
	...
	*-- Memoria (LOOSE - permite llamadas sin configuraci�n expl�cita)
	...
	*-- ServicioPoolDeObjetos
	...
	loSetup = loMock.Setup("ObtenerToken")
	loSetup.Returns("")
	...
	*-- Seguridad, ManagerEntidades y AccionesAutomaticas: Usar clases reales (ya est�n instanciadas en goServicios)
	...
	*-- SetearValorParametro en todos los kontrolers (Factura, Ticket, Remito, Pedido, etc.) accede
	*-- a .oEntidadAccionesAutomaticas como propiedad. No existe en el mock → interceptar via SetupGet.
	loSetup = loMockAccionesAutomaticas.SetupGet("oEntidadAccionesAutomaticas")
	...
	*-- Advertir(tcMensaje, tnBotones, tnBotonDefault, tcTitulo, tnTiempoEspera): captura mensaje y retorna valor
	loSetup = goDovfpMockMensajes.Setup("Advertir")
	loSetup = loSetup.WithCallback(CREATEOBJECT("CallbackAdvertir", goDovfpTestInit.oMockSetupRegistry.oMensajesCapturados))
	loSetup.Returns(1)
	
	*-- Alertar(tcMensaje, tnBotones, tnBotonDefault, tcTitulo, tnTiempoEspera): captura mensaje y retorna valor
	loSetup = goDovfpMockMensajes.Setup("Alertar")
	loSetup = loSetup.WithCallback(CREATEOBJECT("CallbackAlertar", goDovfpTestInit.oMockSetupRegistry.oMensajesCapturados))
	loSetup.Returns(1)
	
	*-- Informar(tcMensaje, tnBotones, tnBotonDefault, tcTitulo, tnTiempoEspera): captura mensaje y retorna valor
	loSetup = goDovfpMockMensajes.Setup("Informar")
	loSetup = loSetup.WithCallback(CREATEOBJECT("CallbackInformar", goDovfpTestInit.oMockSetupRegistry.oMensajesCapturados))
	loSetup.Returns(1)
	
	*-- ManagerLogueos (captura errores logueados para diagn�stico)
	...
	*-- Guardar: No hace nada (el error ya se captur� en Escribir)
	...
	*-- ServicioSaltosDeCampoYValoresSugeridos
	MockServicioSaltosDeCampoYValoresSugeridos()

	*-- PersonalizacionDeEntidades: Usar clase real (ya est� instanciada en goServicios)
	...
	*-- Seguridad
	...
	loSetup = loSetup.WithArgs(goDoVfpMock.oIt.IsAny())
	loSetup.Returns(.T.)
	_screen._instanceFactory.LoadReference('zooinformacion.prg', "Organic.Feline.app", .T.)
	loSetup = loMock.Setup("ObtenerInformacion")
	loSetup.Returns(NEWOBJECT("ZooInformacion", "ZooInformacion.prg"))

	*-- operacionesseguridadadnimplant (PRG stub, necesario para TearDown de tests de seguridad)
...
* Cada callback tiene la firma exacta del m�todo que intercepta
* NOTA: dovfp pasa los argumentos del m�todo mockeado directamente a Invoke()
*-----------------------------------------------------------------------------------------
...
*-- Callback para Advertir(tcMensaje, tcTitulo, tnTipoIcono)
DEFINE CLASS CallbackAdvertir AS Custom
	oMensajesCapturados = .NULL.
...
*-- Callback para Alertar(tcMensaje, tcTitulo, tnTipoIcono)
DEFINE CLASS CallbackAlertar AS Custom
	oMensajesCapturados = .NULL.
...
*-- Callback para Informar(tcMensaje, tcTitulo, tnTipoIcono)
DEFINE CLASS CallbackInformar AS Custom
	oMensajesCapturados = .NULL.
...
*-- Callback para ObtenerObjetoLogueo (captura info de qui�n pide el logueo)
DEFINE CLASS CallbackObtenerObjetoLogueo AS Custom
	...
	FUNCTION Invoke
		...
		*-- No capturamos aqu�, capturamos en MockObjetoLogueo.Escribir()
	ENDFUNC
ENDDEFINE
...
DEFINE CLASS MockObjetoLogueo AS Custom
	...
	*-- M�todo que captura el error cuando se escribe
	...
	*-- M�todos adicionales que puede necesitar ObjetoLogueo
...
* -----------------------------------------------------------------------------------------
function MockServicioSaltosDeCampoYValoresSugeridos()
	*-- ServicioSaltosDeCampoYValoresSugeridos
