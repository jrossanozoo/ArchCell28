...
function Get-BuildContent {
    param($CommonContent, [bool]$IsDebug, [int]$Major, [int]$Minor, [int]$Build)

    if ($IsDebug) {
        return Get-DebugContent -CommonContent $CommonContent
    }

    if (-not [string]::IsNullOrWhiteSpace($env:PBD1)) {
        return Get-ReleaseContent -CommonContent $CommonContent
    }

    if ($Major -eq 1 -and $Minor -eq 1 -and $Build -eq 0) {
        Write-Host "  RELEASE sin variables de entorno, version default ($Major.$Minor.$Build): usando mock"
        return Get-DebugContent -CommonContent $CommonContent
    }

    throw "RELEASE version ${Major}.${Minor}.${Build}: variables de entorno PBD/S vacias. Ejecute desde el pipeline o configure las variables."
}
...
try
{
    ...
    $content = Get-BuildContent -CommonContent $commonContent -IsDebug $isDebug -Major $Major -Minor $Minor -Build $Build
