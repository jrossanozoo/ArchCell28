...
applyTo: ["**/*.vfpsln", "**/*.vfpproj"]
argument-hint: Describe el problema de build o configuración
version: 1.0.0
category: dev
---
...
**Opciones disponibles**:
- `-path`: Ruta al proyecto (.vfpproj), solución (.vfpsln) o directorio
- `-force_download`: 1=forzar descarga, 0=usar cachéé (default)
...
```bash
# Limpiar archivos de compilación (bin/, obj/, packages/) y cachéé NuGet
dovfp clean
...
@workspace DOVFP está fallando con error de dependencias, ayéºdame a diagnosticar
