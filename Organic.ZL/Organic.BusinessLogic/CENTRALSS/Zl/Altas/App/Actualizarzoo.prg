define class ActualizarZoo as Zoosession of Zoosession.prg

	#if .f.
		local this as ActualizarZoo of ActualizarZoo.prg
	#endif
	...
	*-----------------------------------------------------------------------------------------
	function Init() as Void
		dodefault()
		this.oFacturacionLince = newobject( "FacturacionLince", "FacturacionLince.PRG" )
	endfunc 
	...
	*-----------------------------------------------------------------------------------------
	...
	endfunc
	*-----------------------------------------------------------------------------------------
	protected function QuitarCuponesDePresentacion( toRechazo as ItemRechazo of actualizarzoo.prg ) as Integer
	...
	*-----------------------------------------------------------------------------------------
	function LanzarFacturacionEnLince( tcOrigenDeDatos as String, tcRutaLince as String, tdFechaFacturacion as date ) as Collection
		local loInfoProblemasEnFac as Object, loColRetorno as Collection, loColComprobantes as Collection, loColComprobantesNC as Collection
		...
		if empty( tcOrigenDeDatos )
			...
			with this.oFacturacionLince
				...
				loColComprobantes = .GenerarFacturasLinceNuevo( 1 )
				for each loItem in loColComprobantes foxobject
					loColRetorno.add( loItem )
				endfor

				loColComprobantesNC = .GenerarFacturasLinceNuevo( 3 )
				for each loItem in loColComprobantesNC foxobject
					loColRetorno.add( loItem )
				endfor
	...
	*-----------------------------------------------------------------------------------------
	function EliminarCuponesDeRazonesSocialesQueNoSeHanImpactadoenProcesoBancarioenLince( tnNumero as Integer ) as Void
		local lcSql as String , lcCursor as String, lcSqlRS as String, lcCursorRS as String, lnumpres as Integer, lnumpresLINCE as Integer  
		lcCursor   = 'C' + sys(2015) 
		lcCursorRS = 'C' + sys(2015) 
		
		this.AbrirVal(_Screen.Zoo.App.cRutaLince )
		
		*/ Obtengo los nums de presentacion 
		text to lcSql textmerge noshow 
			select distinct (600000000 + Npresen) as NpresenLINCE, Npresen  from zl.Devbco where nrocomprob = <<transform( tnNumero )>>
		endtext 
		This.EjecutarSentencia( lcSql, lcCursor, set("Datasession" ) )
		...
		if used( lcCursor ) and reccount( lcCursor ) > 0
			...
			scan 
				...
				*/ Obtengo las RZSS que NO fueron rechazos en el proceso realizado para excluirlas
			 	lnumpres = &lcCursor..Npresen	
			 	lnumpresLINCE = &lcCursor..NpresenLINCE	
			 	text to lcSqlRS textmerge noshow
				 	select distinct Razsoc as RS 
				 		from zl.Devbco D 
					 	inner join zl.Recmot R on R.ccod = D.Motivo
							where D.nrocomprob = <<transform( tnNumero )>>  
							AND D.Npresen = <<transform( lnumpres )>> 
							and R.EsRechazo != 1	
				endtext
				
				This.EjecutarSentencia( lcSqlRS, lcCursorRS, set("Datasession" ) )		
				
				if used( lcCursorRS ) and reccount( lcCursorRS ) > 0
				
					try
						*/ AGREGO LAS RRSS QUE TENGAN CUPONES EN CADA PRESENTACION QUE LA SUMA DE SUS MONTOS DE 0, PORUQE ESOS NO HAY QQUE ELIMINARLOS  
						select RS from (;
										select upper(alltrim(Val.Jjcli)) as RS ;
											from val ;
											where Val.Jjbjnum = lnumpresLINCE   ;
												and upper( alltrim( Val.Jjcli )) not IN ( select upper(alltrim(RS)) from &lcCursorRS. ) ;
											group by Val.Jjcli ;
											having sum(VAL.Jjm) = 0 ;
										union ;
										select upper(alltrim(RS)) as RS from &lcCursorRS.  ;
										) AS PP;
						into cursor &lcCursorRS.			
						...
						update val set jjbjnum = 0,;
								...
								jjAcCod	=  '', ;
								jjHabil	= 0 ;
							where Val.Jjbjnum = &lcCursor..NpresenLINCE  ;
									and upper( alltrim( Val.Jjcli )) not IN ( select upper(alltrim(RS)) from &lcCursorRS. ) 
						lnRetorno = _tally
						...
						loEx = _screen.zoo.crearObjeto("ZooException")
						with loEx
							.AgregarInformacion( "Error al blanquear cupones sin Registro de proceso bancario de la presentación N° " + ": (" + loError.Message + ")" )
						endwith
						throw loEx
					endtry
				endif	
		...
		use in select( lcCursor )
		use in select( lcCursorRS ) 
	
	endfunc 

	*-----------------------------------------------------------------------------------------
	function GenerarCanjeDeValoresPorSucursalEspecifica(  tcRutaLince as String, tRazonSocial as String ) as String
		local lcComprobante as String, ldFecha as date , lnMontoAjuste as number , lnSelect as Integer, lnNumeroCanje as Number,;
			lnTipoValor as Number, lcCodigoValor as String, lcDescripcion as String, lnImporte as Number, lcVendedor as String 		  
		...
		if empty( tcRutaLince )
		else		
			this.AbrirVal( tcRutaLince )  &&_Screen.Zoo.App.cRutaLince			
			lnNumeroCanje = this.ObtenerNumeroCanje()
			insert into val ;
				(jjNum, jjT, jjCo, jjFe, jjDe, jjNu, jjM, jjCli, jjFecha, jjVen, jjTotFac, jjTurno, jjCotiz);
				values;
			...
			insert into val ;
				...
				values;
				( lnNumeroCanje, lnTipoValor, lcCodigoValor, ldFecha, lcDescripcion, 0,;
				lnImporte * -1, tRazonSocial, ldFecha, lcVendedor, lnImporte, 1, 1)
			...
			lcComprobante = transform( lnNumeroCanje - 800000000 )
		endif  
		return lcComprobante 
