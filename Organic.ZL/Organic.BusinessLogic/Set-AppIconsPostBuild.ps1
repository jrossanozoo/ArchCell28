...
try {
    ...
    # Aplicar ícono a ZL.EXE usando Resource Tuner Console
    $outputDir = Join-Path $ProjectRoot "bin\Exe"
    $zlExe = Join-Path $outputDir "ZL_CORE.EXE"
    
    if (Test-Path $zlExe) {
        Write-Info "Aplicando ícono a ZL.EXE con Resource Tuner Console..."
        ...
        @"
'------------------------------------------------------------------------------
' Auto-generated RTC script: Apply application icon to ZL.EXE
' Timestamp: $timestamp
'------------------------------------------------------------------------------
Sub Main
...
"@ | Out-File $scriptPath -Encoding ASCII
        ...
        try {
            ...
            if ($process.ExitCode -eq 0) {
                Write-Info "✅ Ícono aplicado exitosamente a ZL.EXE"
    ...
    Write-Info "✅ ZL_CORE.exe tiene el ícono embebido automáticamente (via ApplicationIcon)"
